    ...
    return (
        <>
            <div className={styles.links_container}>
                {isEmpty ? <p className={styles.empty_list_message}>No result for {input_reference.current.value.trim()}</p> :
                    Object.keys(filteredLinks).map((category) => (
                        filteredLinks[category].length == 0 ? '' :
                            ...
                            <section key={category} className={`${category}_container`}>
                                ...
                                <h2 className={styles.category_title}>{category == 'beginner' ? 'Beginner essentials' : category.charAt(0).toUpperCase() + category.slice(1)}</h2>
                                <ul className={styles.link_list}>
                                    {filteredLinks[category].map((link) => (
                                        <li key={link.id} className={`${styles.link_item_container} ${category}`}>
                                            <i className={categoryIcons[category] ? categoryIcons[category] : ''}></i>
                                            <a href={link.link} className={styles.link_item} target="_blank" rel="noopener noreferrer">
                                                {link.name}
                                            </a>
                                            {' - ' + link.description}
                                        </li>
